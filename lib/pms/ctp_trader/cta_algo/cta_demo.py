...
class DoubleEmaDemo(CtaTemplate):
    ...
    def __init__(self, cta_engine, setting):
        """Constructor"""
        super(DoubleEmaDemo, self).__init__(cta_engine, setting)
    ...
    def onTick(self, tick=CtaTickData()):
        ...
        # 计算K线
        current_minute = tick.datetime.minute
        
        if current_minute != self.bar_minute:
            if self.bar:
                self.onBar(self.bar)
            bar = CtaBarData()              
            ...
            self.bar_minute = current_minute     # 更新当前的分钟
        ...
        # self.writeCtaLog(''.join([tick.datetime, ' : ', tick.dict_search]))
...
class OrderManagementDemo(CtaTemplate):
    ...
    def __init__(self, cta_engine, setting):
        """Constructor"""
        super(OrderManagementDemo, self).__init__(cta_engine, setting)
