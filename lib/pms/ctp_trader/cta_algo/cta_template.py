...
class CtaTemplate(object):
    ...
    def log_object_info(self, object, keys=None, prefix=str()):
        if not hasattr(object, 'dict_search'):
            self.writeCtaLog(['There exists some error when doing log_object_info, '
                              'probability {} have no attribute "dict_search".'.format(object.__name__)])
        else:
            log_info = object.dict_search(keys=keys)
            for key in keys:
                if key in ['high', 'low', 'open', 'close']:
                    log_info[key] = round(log_info[key], 2)
            self.writeCtaLog(''.join([prefix, log_info.pop('datetime').strftime("%H:%M:%S"), ' : ', str(log_info)]))
            self.putEvent()
